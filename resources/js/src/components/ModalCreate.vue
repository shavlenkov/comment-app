<template>
  <Modal>
    <div id="modalCreate" v-if="show" class="modal fade" data-backdrop="static" aria-hidden="true">
        ...
        <div class="modal-content">
          ...
          <div class="modal-footer" ref="modalFooter">
            ...
            <button :disabled="!success.name || !success.homepage || !success.email || !success.text" type="button" data-dismiss="modal"
                    class="btn btn-primary" @click="createComment">Submit</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    isPreviewDisabled() {
      return !(this.name && this.text && this.gender) || !(this.success.name || this.success.text);
    },
  },
  ...
  methods: {
    ...
    checkHTMLValidity(text) {
      ...
      const stack = [];
      const allowedTags = ['code', 'a', 'i', 'strong'];

      let match;
      while ((match = regex.exec(text)) !== null) {
        const [openingTag, closingTag] = match.slice(1);
        if (allowedTags.includes(openingTag)) {
          stack.push(openingTag.toLowerCase());
        } else if (allowedTags.includes(closingTag)) {
    ...
    checkText() {
      ...
      if (!this.checkHTMLValidity(textCopy) || textCopy === '') {
